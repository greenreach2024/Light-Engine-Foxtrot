...
export async function coreAllocator(zones, iotDevices, ml, options = {}) {
  ...
  if (outdoorContext?.outdoor) {
    ...
    console.log(`[allocator] 🌡  Outdoor: ${o.temperature}°C/${o.humidity}%RH` + 
    ...
    if (multiZoneProblem) {
      ...
      if (shouldEscalate || rhNotImproving) {
        ...
        if (!lastActions[escalateKey] || (now - lastActions[escalateKey]) > cooldown) {
          if (shouldEscalate) {
            console.log(`[allocator]  ESCALATION: RH still high after 15 minutes - activating ALL dehumidifiers`);
          } else {
            console.log(`[allocator]  ESCALATION: No RH improvement detected - activating ALL dehumidifiers`);
          }
        ...
        if (!escalation.singleDehumidifierActive) {
          ...
          console.log(`[allocator] ⏱  Escalation timer: ${elapsed} min elapsed, ${remaining} min remaining (RH: ${escalation.startRH?.toFixed(1)}% → ${problemZone.value.toFixed(1)}%)`);
        }
