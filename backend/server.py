...
async def list_billing_plans() -> JSONResponse:
    ...
    Returns:
        List of plan objects with pricing and limits
    """
    try:
...
async def create_billing_customer(
    ...
    last_name: str = Body(...),
    tenant_id: str = Body(...),
    user: UserContext = Depends(get_current_user) if AUTH_ENABLED else None,
) -> JSONResponse:
    """
    Create a Square customer for billing.
    ...
    Returns:
        Customer details including Square customer_id
    """
    try:
        from backend.billing.square_client import SquareClient
        ...
        if not access_token:
            raise HTTPException(
                status_code=500,
                detail="Square not configured (SQUARE_ACCESS_TOKEN missing)"
            )
        
        square_client = SquareClient(access_token=access_token, environment=environment)
        customer = square_client.create_customer(
            email=email,
...
async def create_subscription(
    ...
    plan_id: str = Body(...),
    card_id: Optional[str] = Body(None),
    user: UserContext = Depends(get_current_user) if AUTH_ENABLED else None,
) -> JSONResponse:
    """
    ...
    Args:
        ...
        card_id: Square card ID for payment
        user: Current authenticated user (if auth enabled)
    ...
    try:
        ...
        if not access_token:
            raise HTTPException(
                status_code=500,
                detail="Square not configured"
            )
        
        square_client = SquareClient(access_token=access_token, environment=environment)
        subscription = square_client.create_subscription(
...
@app.get("/api/billing/usage/{tenant_id}")
async def get_usage_metrics(
    ...
    Returns:
        Usage summary with device count, API calls, storage
    """
    try:
        ...
        tracker = UsageTracker(redis_client=redis_client)
        usage = tracker.get_usage_summary(tenant_id, days=days)
...
@app.get("/api/billing/overage/{tenant_id}")
async def calculate_overage(
    ...
    Returns:
        Overage calculations and charges
    """
    try:
        from backend.billing.usage_tracker import UsageTracker
        from backend.billing.square_client import SquareClient
        ...
        overage = tracker.calculate_overage(tenant_id, plan_id, square_client)
...
@app.post("/api/billing/webhooks/square")
async def handle_square_webhook(
    ...
    Processes subscription and payment lifecycle events.
    
    Args:
        request: FastAPI request object
        x_square_signature: Square webhook signature header
    ...
    try:
        ...
        if os.getenv("SQUARE_ENVIRONMENT") == "production" and x_square_signature:
            notification_url = str(request.url)
            is_valid = handler.verify_signature(payload, x_square_signature, notification_url)
