...
async function handleLogin(e) {
    ...
    // Validation: farmId and password required, email optional
    if (!farmId || !password) {
        showAlert('error', 'Please enter Farm ID and password');
        return;
    }
...
async function loadFarmData() {
    try {
        // Try to load farm profile from API
        ...
        if (data.status === 'success') {
            farmData = data.farm;
            // Also capture groups if returned in profile response (Edge mode)
            if (data.groups) {
                farmData.groups = data.groups;
            }
        } else {
            // Fallback: create basic farmData from farm.json
            console.log('⚠️ Farm profile API failed, loading from farm.json');
            const farmJsonResponse = await fetch('/data/farm.json');
            if (farmJsonResponse.ok) {
                farmData = await farmJsonResponse.json();
            } else {
                // Ultimate fallback
                farmData = { 
                    farmId: currentSession.farmId || 'LOCAL-FARM',
                    name: 'Local Farm'
                };
            }
        }
...
async function loadDashboardData() {
    try {
        ...
        // Initialize farmData if not set
        if (!farmData) {
            farmData = {};
        }
        
        // Load groups data from groups.json if not in farmData
        let groups = Array.isArray(farmData?.groups) ? farmData.groups : [];
        console.log(` farmData.groups exists: ${!!farmData.groups}, length: ${groups.length}`);
        
        if (groups.length === 0) {
            try {
                console.log(' Fetching /data/groups.json...');
                const groupsResponse = await fetch('/data/groups.json');
                console.log(` groups.json response ok: ${groupsResponse.ok}, status: ${groupsResponse.status}`);
                ...
                if (groupsResponse.ok) {
                    ...
                    // Store in farmData for future use
                    farmData.groups = groups;
                } else {
                    console.error(` Failed to load groups.json: ${groupsResponse.status}`);
                }
            } catch (err) {
        ...
        console.log(` Group totals - trays: ${totals.trays}, plants: ${totals.plants}`);
