...
- Broker commission is **12%** (`WHOLESALE_COMMISSION_RATE=0.12`). Collected via Square `app_fee_money` on farm-split payments
...
`app_fee_money` is set on farm-split payments so Square collects GreenReach's 12% commission. On GreenReach direct-charge payments, the full amount goes to GreenReach and commission is tracked via accounting only.
...
2. Loop per farm sub-order:
   - Calculate `brokerFeeCents = Math.round(amountCents * commissionRate)` (12% = 0.12)
   - Create payment on the farm's Square account
   - `app_fee_money` = brokerFeeCents. Square collects 12% for GreenReach
