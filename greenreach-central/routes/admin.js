...
// ─── Farm User Management (admin-protected) ────────────────────────────────

/**
 * GET /api/admin/farm-users
 * List farm_users entries for a given farm
 * Query: ?farm_id=FARM-xxx
 */
router.get('/farm-users', async (req, res) => {
    try {
        const farmId = req.query.farm_id;
        if (!farmId) {
            return res.status(400).json({ success: false, error: 'farm_id query parameter required' });
        }
        if (!(await isDatabaseAvailable())) {
            return res.status(503).json({ success: false, error: 'Database not available' });
        }
        const { rows } = await query(
            `SELECT id, farm_id, email, first_name, last_name, role, status,
                    last_login, created_at, updated_at
             FROM farm_users WHERE farm_id = $1 ORDER BY created_at`,
            [farmId]
        );
        res.json({ success: true, users: rows });
    } catch (error) {
        console.error('[Admin API] Error listing farm users:', error.message);
        res.status(500).json({ success: false, error: 'Failed to list farm users' });
    }
});

/**
 * POST /api/admin/farm-users/reset-password
 * Reset a farm user's password (admin-only)
 * Body: { farm_id, email, new_password }
 */
router.post('/farm-users/reset-password', async (req, res) => {
    try {
        const { farm_id, email, new_password } = req.body || {};
        if (!farm_id || !email || !new_password) {
            return res.status(400).json({
                success: false,
                error: 'farm_id, email, and new_password are required'
            });
        }
        if (!(await isDatabaseAvailable())) {
            return res.status(503).json({ success: false, error: 'Database not available' });
        }
        const hash = await bcrypt.hash(new_password, ADMIN_SALT_ROUNDS);
        const { rowCount } = await query(
            'UPDATE farm_users SET password_hash = $1, updated_at = NOW() WHERE farm_id = $2 AND email = $3',
            [hash, farm_id, email.toLowerCase()]
        );
        if (rowCount === 0) {
            return res.status(404).json({ success: false, error: 'Farm user not found' });
        }
        console.log(`[Admin API] Password reset for ${email} on farm ${farm_id}`);
        res.json({ success: true, message: `Password reset for ${email}` });
    } catch (error) {
        console.error('[Admin API] Error resetting farm user password:', error.message);
        res.status(500).json({ success: false, error: 'Failed to reset password' });
    }
});

/**
 * POST /api/admin/farm-users/create
 * Create or upsert a farm user (admin-only)
 * Body: { farm_id, email, password, role?, first_name?, last_name? }
 */
router.post('/farm-users/create', async (req, res) => {
    try {
        const { farm_id, email, password, role = 'admin', first_name, last_name } = req.body || {};
        if (!farm_id || !email || !password) {
            return res.status(400).json({
                success: false,
                error: 'farm_id, email, and password are required'
            });
        }
        if (!(await isDatabaseAvailable())) {
            return res.status(503).json({ success: false, error: 'Database not available' });
        ...
        await query(
            `INSERT INTO farm_users (id, farm_id, email, first_name, last_name, role, password_hash, status, created_at, updated_at)
             ...
             ON CONFLICT (farm_id, email) DO UPDATE SET
               ...
               updated_at = NOW()`,
            [id, farm_id, email.toLowerCase(), first_name || '', last_name || '', role, hash]
        );
        console.log(`[Admin API] Farm user created/updated: ${email} on farm ${farm_id}`);
        res.json({ success: true, message: `Farm user ${email} created/updated` });
    } catch (error) {
        console.error('[Admin API] Error creating farm user:', error.message);
        res.status(500).json({ success: false, error: 'Failed to create farm user' });
    }
});

/**
 * POST /api/admin/farms/:farmId/mark-setup-complete
 * Mark a farm's setup as completed (admin-only)
 */
router.post('/farms/:farmId/mark-setup-complete', async (req, res) => {
    try {
        const { farmId } = req.params;
        if (!(await isDatabaseAvailable())) {
            return res.status(503).json({ success: false, error: 'Database not available' });
        ...
        console.log(`[Admin API] Marked setup complete for farm ${farmId}`);
        res.json({ success: true, message: `Farm ${farmId} setup marked as complete` });
    } catch (error) {
        console.error('[Admin API] Error marking setup complete:', error.message);
