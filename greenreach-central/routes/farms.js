    ...
    if (dbAvailable) {
      ...
      await query(
        `INSERT INTO farms (farm_id, name, contact_name, status, last_heartbeat, jwt_secret, api_key, api_secret, plan_type, api_url, metadata, updated_at)
         VALUES ($1, COALESCE($2, $1::text), $3, $4, $5, $6, $7, $8, $9, $10, $11, NOW())
         ON CONFLICT (farm_id) 
         DO UPDATE SET 
           status = $4,
           last_heartbeat = $5,
           ...
           name = COALESCE(NULLIF(EXCLUDED.name, $12), farms.name, EXCLUDED.name),
           contact_name = COALESCE(EXCLUDED.contact_name, farms.contact_name),
           updated_at = NOW()`,
        [farmId, metadata?.name || metadata?.farm_name || null, contactName, 'active', now, jwtSecret, apiKey, apiSecret, planType, heartbeatApiUrl, JSON.stringify(metadata || {}), String(farmId)]
      );
