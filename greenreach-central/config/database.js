...
// ─── Migration 042: Research Platform — Study Design & Protocol Management ───
  ...
  // -- Migration 029: Research Integration Layer tables --
  try {
    await pool.query(`
      ...
      CREATE TABLE IF NOT EXISTS instrument_registry (
        ...
        connection_protocol VARCHAR(50) DEFAULT 'manual',
      ...
      CREATE INDEX IF NOT EXISTS idx_instrument_telemetry_farm_inst ON instrument_telemetry(farm_id, instrument_id, recorded_at);
      CREATE INDEX IF NOT EXISTS idx_workflow_runs_farm_status ON workflow_runs(farm_id, run_status);
      CREATE INDEX IF NOT EXISTS idx_approval_gates_farm_status ON approval_gates(farm_id, status);
      CREATE INDEX IF NOT EXISTS idx_immutable_records_farm_type ON immutable_run_records(farm_id, record_type);
      CREATE INDEX IF NOT EXISTS idx_cfd_jobs_farm_status ON cfd_pipeline_jobs(farm_id, status);
    `);
    logger.info('Research integration tables ensured (migration 029)');
  } catch (err) {
    logger.warn('Migration 029 warning:', err.message);
  }
