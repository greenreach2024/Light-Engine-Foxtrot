...
async function runMigrations(client) {
  ...
  try {
    await client.query(`
      ...
      ALTER TABLE farms ADD COLUMN IF NOT EXISTS use_mode VARCHAR(50) DEFAULT NULL;
  ...
  // Wholesale product requests (buyer requests for products not in catalog)
  try {
    await client.query(`
    ...
    `);
  } catch (err) { logger.warn('wholesale_product_requests create warning:', err.message); }
...
// ─── Migration 042: Research Platform — Study Design & Protocol Management ───
  ...
  // --- Migration 062: loss_events and accounting_ledger_entries ---
  try {
    await client.query(`
    ...
    `);
    logger.info('loss_events and accounting_ledger_entries tables ready (migration 062)');
  } catch (err) {
    logger.warn('Migration 062 warning:', err.message);
  }
