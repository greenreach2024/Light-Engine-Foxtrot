...
// Diagnostic: check marketing table existence and create missing ones
app.get('/health/marketing-tables', async (req, res) => {
  try {
    ...
    // Create _gr_uuid if missing
    if (fnResult.rows.length === 0) {
      try {
        await dbQuery(`CREATE OR REPLACE FUNCTION _gr_uuid() RETURNS UUID AS $$ SELECT md5(random()::text || ':' || clock_timestamp()::text)::uuid; $$ LANGUAGE SQL VOLATILE;`);
      } catch (e) { /* ignore */ }
    }

    // Create each missing table individually
    const createResults = {};
    const tableSQL = {
      site_settings: `CREATE TABLE IF NOT EXISTS site_settings (key TEXT PRIMARY KEY, value TEXT NOT NULL, updated_at TIMESTAMPTZ DEFAULT NOW())`,
      marketing_posts: `CREATE TABLE IF NOT EXISTS marketing_posts (
        id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
        platform TEXT NOT NULL CHECK (platform IN ('twitter','linkedin','instagram','facebook')),
        content TEXT NOT NULL, image_url TEXT, hashtags TEXT[] DEFAULT '{}',
        status TEXT NOT NULL DEFAULT 'draft' CHECK (status IN ('draft','approved','scheduled','published','failed','rejected')),
        rejection_reason TEXT,
        source_type TEXT CHECK (source_type IN ('harvest','market','wholesale','sustainability','product','milestone','manual')),
        source_id TEXT, source_context JSONB DEFAULT '{}',
        scheduled_for TIMESTAMPTZ, published_at TIMESTAMPTZ, platform_post_id TEXT,
        metrics JSONB DEFAULT '{"impressions":0,"clicks":0,"likes":0,"shares":0,"comments":0}',
        model_used TEXT, prompt_tokens INTEGER DEFAULT 0, output_tokens INTEGER DEFAULT 0,
        generation_cost_usd NUMERIC(8,6) DEFAULT 0, skill_used TEXT, created_by TEXT, approved_by TEXT,
        created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW()
      )`,
      marketing_post_history: `CREATE TABLE IF NOT EXISTS marketing_post_history (
        id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
        ...
        actor_id TEXT, details JSONB DEFAULT '{}', created_at TIMESTAMPTZ DEFAULT NOW()
      )`,
      marketing_rules: `CREATE TABLE IF NOT EXISTS marketing_rules (
        id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
        rule_name TEXT UNIQUE NOT NULL,
        rule_type TEXT NOT NULL CHECK (rule_type IN ('auto_approve','always_block','rate_limit','content_filter','skill_gate')),
        conditions JSONB DEFAULT '{}', enabled BOOLEAN DEFAULT true,
        created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW()
      )`,
      marketing_skills: `CREATE TABLE IF NOT EXISTS marketing_skills (
        id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
        skill_name TEXT UNIQUE NOT NULL, description TEXT,
        ...
        allowed_actions TEXT[] DEFAULT '{}', blocked_actions TEXT[] DEFAULT '{}', system_prompt TEXT,
        enabled BOOLEAN DEFAULT true, created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW()
      )`,
      farm_delivery_zones: `CREATE TABLE IF NOT EXISTS farm_delivery_zones (
        id SERIAL PRIMARY KEY, farm_id VARCHAR(255) NOT NULL, zone_id VARCHAR(50) NOT NULL,
        name VARCHAR(255) NOT NULL, description TEXT DEFAULT '', fee NUMERIC(10,2) DEFAULT 0,
        min_order NUMERIC(10,2) DEFAULT 25, postal_prefix VARCHAR(10),
    ...
    for (const [name, sql] of Object.entries(tableSQL)) {
      try {
        await dbQuery(sql);
        createResults[name] = 'created';
      } catch (e) {
        createResults[name] = e.message;
      }
    }

    // Create indexes
    try {
      await dbQuery(`CREATE INDEX IF NOT EXISTS idx_marketing_posts_status ON marketing_posts(status)`);
      await dbQuery(`CREATE INDEX IF NOT EXISTS idx_marketing_posts_platform ON marketing_posts(platform)`);
      await dbQuery(`CREATE INDEX IF NOT EXISTS idx_marketing_posts_created ON marketing_posts(created_at)`);
      await dbQuery(`CREATE INDEX IF NOT EXISTS idx_marketing_post_history_post ON marketing_post_history(post_id)`);
      await dbQuery(`CREATE INDEX IF NOT EXISTS idx_marketing_rules_enabled ON marketing_rules(enabled) WHERE enabled = true`);
      await dbQuery(`CREATE INDEX IF NOT EXISTS idx_marketing_skills_enabled ON marketing_skills(enabled) WHERE enabled = true`);
      await dbQuery(`CREATE INDEX IF NOT EXISTS idx_farm_delivery_zones_farm ON farm_delivery_zones(farm_id)`);
      await dbQuery(`CREATE INDEX IF NOT EXISTS idx_farm_delivery_zones_status ON farm_delivery_zones(status)`);
      createResults.indexes = 'created';
    } catch (e) {
      createResults.indexes = e.message;
    }
    ...
    try {
      ...
      `);
      createResults.rules_seed = 'seeded';
    } catch (e) {
      createResults.rules_seed = e.message;
    }
    ...
    try {
      await dbQuery(`
        INSERT INTO marketing_skills (skill_name, description, category, risk_tier, approval_mode, allowed_actions, blocked_actions) VALUES
          ('content-drafter', 'Draft social media content from farm data, market intelligence, and seasonal context', 'content', 2, 'required', ARRAY['draft-caption','generate-calendar','repurpose-content','summarize-performance'], ARRAY['publish-post','send-direct-message','invent-testimonials']),
          ('compliance-screener', 'Check content against CFIA food marketing regulations and brand policy', 'compliance', 3, 'required', ARRAY['flag-risky-claims','block-draft','route-to-human-review'], ARRAY['override-human-decision','rewrite-policy']),
          ('analytics-summarizer', 'Summarize post performance metrics and engagement data', 'analytics', 0, 'none', ARRAY['summarize-metrics','compare-periods','surface-anomalies'], ARRAY['change-tracking','publish-external-report']),
          ('engagement-responder', 'Draft responses to social media interactions', 'engagement', 2, 'required', ARRAY['draft-reply','suggest-escalation','classify-sentiment'], ARRAY['send-external-message','change-account-status']),
          ('schedule-optimizer', 'Recommend optimal posting times from engagement data', 'scheduling', 1, 'spot-check', ARRAY['recommend-time','analyze-engagement-patterns','suggest-frequency'], ARRAY['auto-schedule-without-review','delete-scheduled-posts']),
          ('content-planner', 'Build content themes, weekly calendars, and test ideas based on campaign goals and performance data', 'content', 1, 'spot-check', ARRAY['build-calendar','suggest-topics','plan-test','audit-content-mix'], ARRAY['publish-post','modify-campaign']),
          ('learning-engine', 'Convert performance outcomes into updated strategy patterns and learning records', 'analytics', 0, 'none', ARRAY['update-patterns','retire-patterns','generate-learning-record','recommend-next-test'], ARRAY['override-brand-rules','auto-apply-strategy']),
          ('blog-writer', 'Draft blog articles optimized for clarity, search intent, and downstream social repurposing', 'content', 2, 'required', ARRAY['draft-article','suggest-internal-links','extract-social-posts','outline-article'], ARRAY['publish-article','invent-data'])
        ON CONFLICT (skill_name) DO NOTHING
      `);
      createResults.skills_seed = 'seeded';
    } catch (e) {
      createResults.skills_seed = e.message;
    }

    // Re-check tables
    const result = await dbQuery(`SELECT tablename FROM pg_tables WHERE schemaname = 'public' AND tablename IN ('marketing_posts','marketing_rules','marketing_skills','marketing_post_history','site_settings','farm_delivery_settings','farm_delivery_zones','farm_delivery_windows') ORDER BY tablename`);
    const found = result.rows.map(r => r.tablename);

    res.json({
      status: found.length >= 8 ? 'ok' : 'partial',
