...
app.use((req, res, next) => {
  ...
  if (reqPath === '/LE-dashboard.html' || reqPath === '/farm-admin.html' || reqPath === '/wholesale.html') return next();
...
//
// Also reconciles groups.json in the Central farm_data DB from each room's
// installedSystems so the 3D viewer reflects the build plan. Without this,
// groups.json stays frozen at whatever was seeded months ago (e.g. 78 ZipGrow
// entries lingering after build plan changed to 96 NFT racks) and the viewer
// shows a catastrophic mismatch between rooms and grow systems.
app.post('/api/setup/save-rooms', authMiddleware, (req, res) => {
  // Fire-and-forget: do not block the LE proxy response on reconcile.
  reconcileGroupsFromRoomsPayload(req).catch((err) => {
    logger.warn('[save-rooms] Group reconcile failed:', err?.message || err);
  });
  return proxyToLE(req, res, '/api/setup/save-rooms');
});

// Load grow-systems.json once; it's static tooling metadata.
let __growSystemsCatalog = null;
function getGrowSystemsCatalog() {
  if (__growSystemsCatalog) return __growSystemsCatalog;
  try {
    const p = path.join(__dirname, 'public/data/grow-systems.json');
    __growSystemsCatalog = JSON.parse(fs.readFileSync(p, 'utf8'));
  } catch (err) {
    logger.warn('[save-rooms] Could not load grow-systems catalog:', err?.message || err);
    __growSystemsCatalog = { templates: [] };
  }
  return __growSystemsCatalog;
}

/**
 * Rebuild the groups list for each room in the payload from its
 * installedSystems, then persist the combined groups array to the Central
 * farm_data DB row for (farmId, data_type='groups'). Groups for rooms NOT in
 * the payload are preserved. Crop / plan / schedule / status / lastModified
 * fields are copied forward when a group with the same id already exists.
 */
async function reconcileGroupsFromRoomsPayload(req) {
  const farmId = req.farmId || process.env.FARM_ID || 'default';
  const rooms = Array.isArray(req.body?.rooms) ? req.body.rooms : [];
  if (!rooms.length) return;

  const catalog = getGrowSystemsCatalog();
  ...
  if (isDatabaseAvailable()) {
    try {
      ...
      if (r.rows.length && r.rows[0].data != null) {
        const d = r.rows[0].data;
        existing = Array.isArray(d) ? d : (d.groups || []);
      }
    } catch (err) {
      logger.warn('[save-rooms] reconcile: DB read failed:', err?.message || err);
    }
  }
  ...
  const roomKeysInPayload = new Set();
  for (const r of rooms) {
    if (r?.id) roomKeysInPayload.add(String(r.id));
    if (r?.name) roomKeysInPayload.add(String(r.name));
  }
  ...
  // Preserve groups belonging to rooms NOT in this payload.
  for (const g of existing) {
    if (!g) continue;
    const gRoom = g.room || g.roomName || '';
    if (!roomKeysInPayload.has(String(gRoom))) {
      next.push(g);
    }
  }

  // Write back to DB + in-memory store.
  if (isDatabaseAvailable()) {
    try {
      await query(
        `INSERT INTO farm_data (farm_id, data_type, data, updated_at)
        ...
        [farmId, JSON.stringify(next)]
      );
      logger.info(`[save-rooms] Reconciled ${next.length} groups for farm ${farmId} from ${rooms.length} room(s)`);
    } catch (err) {
      logger.error('[save-rooms] reconcile: DB write failed:', err?.message || err);
    }
  }
  if (_inMemoryStore) {
    if (!_inMemoryStore.groups) _inMemoryStore.groups = new Map();
    _inMemoryStore.groups.set(farmId, next);
  }
}
