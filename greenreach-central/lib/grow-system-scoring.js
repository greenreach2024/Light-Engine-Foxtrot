...
export function scoreAirflowDeficit(room, achMap = DEFAULT_ENVELOPE_ACH) {
  ...
  if (!Number.isFinite(requiredCFM) || requiredCFM <= 0) {
    // roomVolumeM3() accepts snake_case dimension keys but computeSupplyFanCFM()
    // only reads camelCase, so a snake_case room can pass the volume guard and
    // still have no required CFM. Treat as unknown rather than dividing by zero.
    return {
      score: 50,
      requiredCFM: 0,
      supplyCFM: null,
      volumeSource: 'unknown',
      rationale: 'Could not compute required CFM from room dimensions; returning neutral score.'
    };
  }
