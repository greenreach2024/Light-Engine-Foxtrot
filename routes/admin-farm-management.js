...
router.post('/auth/login', async (req, res) => {
  try {
    ...
    const FALLBACK_ADMIN = {
      email: 'admin@greenreachgreens.com',
      password: 'Admin2025!',
...
router.post('/users', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    if (!db || !db.pool || db.mode === 'nedb') {
      ...
      // Also verify CC email (admin@greenreachgreens.com)
      console.log('[Admin] Auto-verifying CC email (admin@greenreachgreens.com) in SES...');
      const ccVerification = await verifySESRecipient('admin@greenreachgreens.com');
      ...
      if (sesVerification.verified && ccVerification.verified) {
        ...
        if (ccVerification.needs_confirmation) pendingEmails.push('admin@greenreachgreens.com');
    ...
    try {
      ...
      console.log('[Admin] CC: admin@greenreachgreens.com');
    ...
    res.json({ 
      ...
      message: emailSent 
        ? `User ${action} successfully and welcome email sent to ${normalizedEmail} (CC: admin@greenreachgreens.com)` 
        : verificationPending
    ...
    <div style="font-family: system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; background: #ffffff;">
      ...
      <div style="padding: 32px 24px;">
        ...
        <ul style="margin: 0 0 24px 0; padding-left: 20px; color: #525252; font-size: 13px; line-height: 1.6;">
          <li style="margin-bottom: 4px;">Email: <a href="mailto:admin@greenreachgreens.com" style="color: #171717; text-decoration: underline;">admin@greenreachgreens.com</a></li>
