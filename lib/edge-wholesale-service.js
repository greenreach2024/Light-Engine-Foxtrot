/**
 * Edge Wholesale Integration Service
 * 
 * Syncs farm inventory to GreenReach Central for wholesale catalog aggregation.
 * Runs in edge mode only - pushes local inventory data to Central API.
 * 
 * Features:
...
class EdgeWholesaleService {
  ...
  start() {
    if (!edgeConfig.isEdgeMode()) {
      console.log('[Wholesale Sync] Not in edge mode, service disabled');
      return;
    }
