...
export class DeviceWizard {
  ...
  async saveIntegration() {
    ...
    // Phase 2 Task 2.7: Auto-assign discovered device to group in groups.json
    if (record.group_id && this.options.groupsFilePath) {
      try {
        const groupsPath = this.options.groupsFilePath;
        const groupsData = JSON.parse(readFileSync(groupsPath, 'utf8'));
        const targetGroup = groupsData.find(g => g.id === record.group_id);
        if (targetGroup) {
          const deviceId = record.config?.ip
            || record.config?.host
            || record.device_make_model
            || record.driver_id;
          const deviceEntry = {
            id: deviceId,
            name: this.state.selectedDevice?.name || deviceId,
            vendor: this.state.selectedDevice?.manufacturer || record.protocol,
            protocol: record.protocol,
            integration_id: record._id || null,
            auto_assigned: true,
            assigned_at: new Date().toISOString()
          };
          // Add to lights array (create if missing)
          if (!Array.isArray(targetGroup.lights)) targetGroup.lights = [];
          const alreadyAssigned = targetGroup.lights.some(l => l.id === deviceId);
          if (!alreadyAssigned) {
            targetGroup.lights.push(deviceEntry);
            // Keep members/devices arrays in sync
            if (!Array.isArray(targetGroup.members)) targetGroup.members = [];
            if (!targetGroup.members.includes(deviceId)) targetGroup.members.push(deviceId);
            if (!Array.isArray(targetGroup.devices)) targetGroup.devices = [];
            if (!targetGroup.devices.includes(deviceId)) targetGroup.devices.push(deviceId);
            targetGroup.deviceCount = (targetGroup.deviceCount || 0) + 1;
            writeFileSync(groupsPath, JSON.stringify(groupsData, null, 2));
            console.log(`[device-wizard] Auto-assigned device ${deviceId} to group ${record.group_id}`);
          } else {
            console.log(`[device-wizard] Device ${deviceId} already in group ${record.group_id}`);
          }
        } else {
          console.warn(`[device-wizard] Group ${record.group_id} not found in groups.json`);
        }
      } catch (groupErr) {
        console.warn(`[device-wizard] Auto-assign to group failed (non-fatal): ${groupErr.message}`);
      }
    }
  ...
  return {
    ...
    start: async (req, res) => {
      ...
      const wizard = new DeviceWizard({
        integrationDB,
        groupsFilePath: join(process.cwd(), 'public', 'data', 'groups.json'),
