 ...
 * 
 * Manages configuration for farm server operation including:
 * - Farm server vs hosted mode detection
...
const DEFAULT_CONFIG = {
  ...
  farmName: null,
  apiKey: null,
  centralApiUrl: process.env.CENTRAL_API_URL || 'https://www.greenreachgreens.com',
...
class EdgeConfig {
  ...
  loadConfig() {
    try {
      if (fs.existsSync(CONFIG_FILE)) {
        ...
        console.log(`✓ Loaded farm configuration: ${this.config.mode} mode`);
      }
    } catch (error) {
      console.warn('Warning: Could not load farm config, using defaults:', error.message);
    }
  }
  ...
  saveConfig() {
    try {
      ...
      console.log('✓ Saved farm configuration');
    } catch (error) {
  ...
  /**
   * Check if running in farm server mode
   */
  isEdgeMode() {
    // Debug logging
    console.log('[EdgeConfig] DEBUG: process.env.EDGE_MODE =', process.env.EDGE_MODE);
    console.log('[EdgeConfig] DEBUG: this.config.mode =', this.config.mode);
    
    // Environment variable takes precedence over config file
    if (process.env.EDGE_MODE !== undefined) {
      const result = process.env.EDGE_MODE === 'true';
      console.log('[EdgeConfig] Using environment variable, isEdgeMode =', result);
      return result;
    }
    const result = this.config.mode === 'edge';
    console.log('[EdgeConfig] Using config file, isEdgeMode =', result);
    return result;
  }
  ...
  reset() {
    this.config = { ...DEFAULT_CONFIG };
    this.saveConfig();
    console.log('✓ Farm configuration reset to defaults');
  }
}
