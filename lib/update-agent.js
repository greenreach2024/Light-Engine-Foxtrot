/**
 * Update Agent
 * Automatic update system for Light Engine farm servers and desktop apps
 * 
 * Features:
...
export class UpdateAgent {
  ...
  async verifySignature(filePath, signatureUrl) {
    try {
      ...
      // Load the public key — look for embedded key first, then config file
      const pubKeyPaths = [
        path.join(__dirname, '..', 'config', 'update-public-key.pem'),
        path.join(__dirname, '..', 'data', 'update-public-key.pem'),
        path.join(__dirname, 'update-public-key.pem')
      ];
      let publicKey = null;
      for (const p of pubKeyPaths) {
        try { publicKey = fs.readFileSync(p, 'utf-8'); break; } catch (_) { /* next */ }
      }
      if (!publicKey) {
        console.warn('[UpdateAgent] No public key found — signature verification skipped (install a PEM key to enable)');
        return true; // Graceful degradation until key is provisioned
      }
      ...
      const valid = verifier.verify(publicKey, sigData);

      if (!valid) {
        console.error('[UpdateAgent] ❌ RSA signature verification FAILED');
      } else {
        console.log('[UpdateAgent] ✅ RSA signature verified');
      }
      return valid;
    } catch (err) {
      console.error('[UpdateAgent] Signature verification error:', err.message);
      return false;
    }
  }
