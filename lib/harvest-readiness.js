 ...
 *
 * Phase 3 T30: Now also incorporates AI vision QA checkpoint health_score data
 * when available, blending it with experiment record quality scores.
 *
 * @param {string} crop - crop name
 * @param {Array} experimentRecords - historical experiment records
 * @param {Array} [qaCheckpoints] - optional AI vision QA checkpoints with health_score + grow_day
 */
export function analyzeQualityTrend(crop, experimentRecords, qaCheckpoints = []) {
  const cropKey = (crop || '').toLowerCase();
  const relevant = experimentRecords
    .filter(r => (r.crop || '').toLowerCase().includes(cropKey) &&
  ...
  // Merge AI vision QA checkpoint health_scores (0-100 → normalized to same scale)
  const visionEntries = (qaCheckpoints || [])
    .filter(cp => {
      const m = cp.metrics ? (typeof cp.metrics === 'string' ? JSON.parse(cp.metrics) : cp.metrics) : null;
      return m?.health_score != null && cp.grow_day != null;
    })
    .map(cp => {
      const m = typeof cp.metrics === 'string' ? JSON.parse(cp.metrics) : cp.metrics;
      return { grow_days: cp.grow_day, quality_score: m.health_score, source: 'ai_vision' };
    });

  // Combine experiment records and vision checkpoints
  const combined = [
    ...relevant.map(r => ({ grow_days: r.grow_days, quality_score: r.outcomes.quality_score, source: 'experiment' })),
    ...visionEntries
  ].sort((a, b) => a.grow_days - b.grow_days);

  if (combined.length < READINESS_CONFIG.min_weight_samples) {
    return { optimalDay: null, avgQuality: null, pattern: 'insufficient_data', sampleCount: combined.length, visionSamples: visionEntries.length };
  }

  // Find the grow_days that maximizes quality
  let bestDay = combined[0].grow_days;
  let bestQuality = combined[0].quality_score;

  const qualityByDay = {};
  for (const r of combined) {
    const dayBucket = Math.round(r.grow_days);
    if (!qualityByDay[dayBucket]) qualityByDay[dayBucket] = [];
    qualityByDay[dayBucket].push(r.quality_score);
  }
  ...
  const avgQuality = combined.reduce((s, r) => s + r.quality_score, 0) / combined.length;

  return {
    optimalDay: bestDay,
    peakQuality: +bestQuality.toFixed(1),
    avgQuality: +avgQuality.toFixed(1),
    pattern: bestDay < combined[combined.length - 1].grow_days ? 'peaks_then_declines' : 'improves_with_time',
    sampleCount: combined.length,
    visionSamples: visionEntries.length
  };
}
