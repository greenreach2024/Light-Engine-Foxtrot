...
export function getCurrentRecipe(plan, config, currentDate = new Date()) {
  ...
  // ---- CUT AND COME AGAIN: during regrowth, hold at last vegetative recipe ----
  const harvestCycle = config?.harvestCycle;
  if (harvestCycle && harvestCycle.strategy === 'cut_and_come_again' &&
      harvestCycle.currentHarvest > 0 && harvestCycle.currentHarvest < harvestCycle.maxHarvests) {
    // Find the last Vegetative stage entry in the plan (regrowth lighting)
    ...
    // Find last vegetative entry; fall back to last entry before Harvest/Maturation stage
    let vegEntry = null;
    for (const row of normalized) {
      if (row.stage.includes('vegetat') || row.stage.includes('growth') || row.stage.includes('veg')) {
        vegEntry = row.entry;
      }
    }
    // Fallback: use the entry at ~60-70% of plan length (typical late vegetative)
    if (!vegEntry && normalized.length > 2) {
      const idx = Math.floor(normalized.length * 0.65);
      vegEntry = normalized[idx]?.entry;
    }
    if (!vegEntry) vegEntry = normalized[Math.max(0, normalized.length - 2)]?.entry;

    if (vegEntry) {
      const result = vegEntry.mix ? { ...vegEntry.mix, stage: 'Regrowth' } : { ...vegEntry, stage: 'Regrowth' };
      return result;
    }
  }
