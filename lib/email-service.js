...
export async function sendEmail({ to, subject, text, html, from }) {
  ...
  try {
    switch (EMAIL_PROVIDER) {
      case 'ses':
        return await sendViaSES({ to, subject, text, html, from: fromAddress });
      
      case 'sendgrid':
        return await sendViaSendGrid({ to, subject, text, html, from: fromAddress });
      
      case 'smtp':
        return await sendViaSMTP({ to, subject, text, html, from: fromAddress });
...
async function sendViaSendGrid({ to, subject, text, html, from }) {
  ...
  console.log('[email] SendGrid send success:', { to });
