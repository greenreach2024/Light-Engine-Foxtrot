...
const FROM_EMAIL = process.env.FROM_EMAIL || process.env.EMAIL_FROM || 'noreply@farm.local';
const FROM_NAME = process.env.FROM_NAME || process.env.EMAIL_FROM_NAME || 'Light Engine Foxtrot';
...
async function initSendGrid() {
  ...
  if (EMAIL_PROVIDER !== 'sendgrid') return null;
...
// Nodemailer configuration (SMTP primary or SES fallback)
...
export async function sendEmail({ to, subject, text, html, from, cc, bcc }) {
  ...
  try {
    ...
    const isSandbox = error.message && (error.message.includes('not verified') || error.message.includes('MessageRejected'));
    if (isSandbox && EMAIL_PROVIDER === 'ses' && nodemailerTransport) {
      console.log(`[email] SES sandbox blocked ${to}, falling back to SMTP`);
      return await sendViaSMTP({ to, subject, text, html, from: fromAddress, cc, bcc });
    }
    if (EMAIL_PROVIDER === 'ses' && nodemailerTransport) {
      console.log(`[email] SES failed, falling back to SMTP for ${to}`);
      try {
        return await sendViaSMTP({ to, subject, text, html, from: fromAddress, cc, bcc });
      } catch (smtpErr) {
