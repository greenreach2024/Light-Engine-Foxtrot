  ...
  const farmHTML = farms.map(farm => {
    ...
    const itemRows = farm.items.map(item => {
      ...
      const discountNote = item.discount_rate > 0
        ...
        : '';
      const descRow = item.description ? `<div style="font-size:11px;color:#6b7280;margin-top:2px;">${esc(item.description)}</div>` : '';
      const taxBadge = item.is_taxable === false ? ' <span style="font-size:10px;color:#9ca3af;font-style:italic;">Tax-exempt</span>' : '';
      return `
        <tr>
          <td>${esc(item.sku_name)}${taxBadge}${descRow}</td>
