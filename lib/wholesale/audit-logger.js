...
class AuditLogger {
  ...
  async log(entityType, entityId, action, oldValue, newValue, userId = null, metadata = {}) {
    try {
      ...
      if (this.logs.length > this.maxLogsInMemory) {
        this.logs.shift();
        console.warn(` Audit log exceeded ${this.maxLogsInMemory} entries, trimmed oldest`);
      }
  ...
  clearOldLogs(olderThanDays) {
    ...
    console.log(`🗑 Cleared ${beforeCount - afterCount} audit logs older than ${olderThanDays} days`);
