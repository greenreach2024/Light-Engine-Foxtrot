 ...
 * Refuses to confirm if the reservation has already expired (TTL enforcement).
 * @param {string} reservationId
 * @returns {number} Number of updated docs (0 if reservation is missing or expired)
 */
export async function confirmReservation(reservationId) {
  const now = new Date().toISOString();
  // Only confirm rows that are still active AND not yet expired.
  const updated = await reservationsDB.update(
    { reservation_id: reservationId, status: 'active', expires_at: { $gt: now } },
    { $set: { status: 'confirmed', updated_at: new Date().toISOString() } }
  );
  if (!updated) {
    const existing = await reservationsDB.findOne({ reservation_id: reservationId });
    if (existing && existing.status === 'active' && existing.expires_at <= now) {
      console.warn(`[reservation-store] Refused to confirm expired reservation ${reservationId} (expired ${existing.expires_at})`);
    }
  }
  return updated;
}
 ...
 * Only confirms active, non-expired rows. Expired rows stay 'active' until the
 * cleanup job flips them to 'expired' so TTL is never silently bypassed.
 * @param {string} orderId
 * @returns {number} Number of updated docs
 */
export async function confirmOrderReservations(orderId) {
  const now = new Date().toISOString();
  return reservationsDB.update(
    { order_id: orderId, status: 'active', expires_at: { $gt: now } },
    { $set: { status: 'confirmed', updated_at: new Date().toISOString() } },
    { multi: true }
  );
}
