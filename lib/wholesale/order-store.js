...
// ─── Order Audit Logs ────────────────────────────────────────────────
const auditLogsDB = createStore('data/wholesale-audit-logs.db');
auditLogsDB.ensureIndex({ fieldName: 'order_id' });
auditLogsDB.ensureIndex({ fieldName: 'farm_id' });
auditLogsDB.ensureIndex({ fieldName: 'action' });
auditLogsDB.persistence.setAutocompactionInterval(600000);
...
// Used to prevent Central fallback from reverting accepted/modified orders to pending.
const STATUS_RANK = {
...
/** Save or upsert a sub-order. Preserves existing status if it is more advanced. */
export async function saveSubOrder(subOrder) {
  const existing = await subOrdersDB.findOne({ sub_order_id: subOrder.sub_order_id });
  if (existing) {
    // Never regress status — if existing status is more advanced, keep it
    const merged = { ...subOrder, updated_at: new Date().toISOString() };
    if (statusRank(existing.status) > statusRank(subOrder.status)) {
      merged.status = existing.status;
    }
    await subOrdersDB.update({ _id: existing._id }, { $set: merged });
...
/** Log an order action to the persistent audit trail */
export async function logOrderAction(orderId, farmId, action, details = {}, performedBy = null) {
  return auditLogsDB.insert({
    order_id: orderId,
    farm_id: farmId,
    action,
    details,
...
export async function getOrderAuditLogs(orderId, limit = 100) {
  return auditLogsDB.find({ order_id: orderId }).sort({ created_at: -1 }).limit(limit);
}
