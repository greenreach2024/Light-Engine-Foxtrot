...
export async function allocateOrder(cart, catalog, options = {}) {
  ...
  for (const cartItem of cart.items) {
    ...
    for (const farmAvailability of sortedFarms) {
      ...
      // Calculate line total (apply buyer volume discount)
      const effective_price = buyer_discount_rate > 0
        ? Math.round(farmAvailability.price_per_unit * (1 - buyer_discount_rate) * 100) / 100
        : farmAvailability.price_per_unit;
      const line_total = allocateQty * effective_price;
      ...
      subOrder.line_items.push({
        ...
        list_price: farmAvailability.price_per_unit,
        discount_rate: buyer_discount_rate,
        line_total,
        is_taxable: catalogItem.is_taxable !== false,
  ...
  const subOrders = Array.from(allocations.values()).map(subOrder => {
    ...
    // Per-item tax: only tax line items where is_taxable !== false
    const taxable_subtotal = subOrder.line_items
      .filter(li => li.is_taxable !== false)
      .reduce((sum, li) => sum + li.line_total, 0);
    subOrder.tax_amount = Math.round(taxable_subtotal * tax_rate * 100) / 100;
