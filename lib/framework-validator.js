        ...
        if (file.includes('groups.json')) {
          ...
          if (!groups) {
            ...
            groups.forEach((group, idx) => {
              // status and active may be absent in cloud-restored data — treat as defaults
              if (group.status === null) {
                this.violations.push(
                  `⚠️ groups.json[${idx}] ID:"${group.id}" has null status — defaulting to 'growing'`
                );
              }
              // active=undefined is normal for cloud-restored groups (implicitly active)
            });
          }
        }
      } catch (error) {
