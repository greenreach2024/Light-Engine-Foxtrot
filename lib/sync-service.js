...
class SyncService {
  ...
  start() {
    ...
    // LE is authoritative: never restore from Central, which holds a stale
    // farm_data copy that diverges from the GCS-persisted flat files.
    // When the farm first registers, Central has no data, so restore would wipe
    // operator edits. The authoritative flag in edge-config.json encodes this
    // permanently — it is not a temporary workaround.
    this.edgeConfig = edgeConfig;
    const isAuthoritative = this.edgeConfig.config?.authoritative === true;

    if (!isAuthoritative) {
      // Non-authoritative mode: restore initial data from Central.
      // Only relevant for read-only replica deployments.
  ...
  async registerFarm() {
    try {
      ...
      // Include pickup/delivery schedule from saved farm settings so invoices show correct instructions
      if (!farmData.fulfillment_standards) {
        let savedStandards = null;
        try {
          const settingsPath = path.join(__dirname, '..', 'data', 'farm-settings.json');
          const settingsRaw = await fs.promises.readFile(settingsPath, 'utf8');
          const savedSettings = JSON.parse(settingsRaw);
          if (savedSettings.fulfillmentStandards && Object.keys(savedSettings.fulfillmentStandards).length > 0) {
            savedStandards = savedSettings.fulfillmentStandards;
          }
        } catch (_) { /* settings file may not exist yet */ }
        farmData.fulfillment_standards = savedStandards || {};
      }
  ...
  async pushFullBackup() {
    ...
    try {
      // NOTE: rooms and groups are intentionally NOT synced to Central here.
      // LE is the canonical store for both (GCS-FUSE backed). Central's GET
      // path for rooms.json and groups.json proxies to LE. If rooms/groups
      // were pushed to Central's farm_data table, re-enabling farmDataMiddleware
      // for those keys would silently serve stale data again.
