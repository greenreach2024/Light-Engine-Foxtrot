...
class SyncService {
  ...
  async registerFarm() {
    try {
      ...
      // Add API URL: prefer explicit env var (EB public URL), fall back to local IP
      farmData.api_url = process.env.FARM_API_URL || `http://${this.getLocalIP()}:${process.env.PORT || 8091}`;
      ...
      const response = await axios.post(
        `${centralUrl}/api/farms/${farmId}/heartbeat`,
        {
          ...
          metadata: {
            ...
            plan_type: 'edge',
            api_url: process.env.FARM_API_URL || `http://${this.getLocalIP()}:${process.env.PORT || 8091}`
          }
        },
        {
          headers: {
