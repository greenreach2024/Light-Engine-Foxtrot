  ...
  async sendHeartbeat() {
    try {
      ...
      const response = await axios.post(
        `${centralUrl}/api/farms/${farmId}/heartbeat`,
        {
          ...
          metadata: {
            ...
            plan_type: 'edge',
            api_url: `http://${this.getLocalIP()}:${process.env.PORT || 8091}`
          }
        },
        {
          headers: {
      ...
      if (this.edgeConfig?.config?.authoritative === true) {
        console.warn('⚠️  [sync-service] PROTECTED MODE: Edge authoritative flag active - Central data not syncing to edge');
      }
    } catch (error) {
