...
function calculateStability(zone) {
  ...
  for (const metric of metrics) {
    const sensor = sensors[metric];
    
    if (!sensor) continue;
    
    // Check if history exists and has enough data
    let history = sensor.history;
    
    // If no history array, try to use current value as single-point history
    if (!history || !Array.isArray(history) || history.length === 0) {
      if (sensor.current !== null && sensor.current !== undefined) {
        // Not enough data for variance calculation, assume stable
        totalStability += 1;
        count++;
      }
      continue;
    ...
    if (history.length < 5) {
      // Not enough readings for proper variance calculation
      // Assume decent stability if we have some data
      totalStability += 0.8;
      count++;
      continue;
    }
    ...
    // Support both array of values and array of objects with 'value' property
    const values = history.map(h => {
      if (typeof h === 'object' && h !== null) {
        return h.value !== undefined ? h.value : h;
      }
      return h;
    }).filter(v => v !== null && v !== undefined && !isNaN(v));
    
    if (values.length === 0) {
      // Have history but no valid values - treat as moderately stable
      totalStability += 0.7;
      count++;
      continue;
    }
  ...
  return count > 0 ? (totalStability / count) * 100 : 80; // Default 80 if no data (neutral)
}
...
function calculateSensorReliability(zone) {
  ...
  let recentCount = 0;
  // Sensors typically report every 5-15 minutes, so 30 minutes is a reasonable threshold
