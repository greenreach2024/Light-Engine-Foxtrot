...
const growSystemsSchema = {
  ...
  properties: {
    ...
    lightingSizingRule: {
      type: 'object',
      description: 'Global rule for sizing LED fixture wattage from canopy area, plus per-template exceptions.',
      required: ['wattsPerFt2', 'exceptions'],
      additionalProperties: true,
      properties: {
        wattsPerFt2: { type: 'number', exclusiveMinimum: 0 },
        canopyArea: { type: 'string' },
        exceptions: {
          type: 'array',
          uniqueItems: true,
          items: { type: 'string' }
    ...
    templates: {
      ...
      items: {
        ...
        properties: {
          ...
          plantLocations: {
            type: 'object',
            description: 'Authoritative user-facing plant count per cropClass. UI labels and build planning read this in preference to tierCount x traysPerTier x plantsPerTrayByClass when present.',
            required: ['totalByClass'],
            additionalProperties: true,
            properties: {
              totalByClass: {
                type: 'object',
                description: 'Total plant locations for the whole template, keyed by cropClass. Sparse map — only classes this template is suitable for need a value.',
                additionalProperties: false,
                minProperties: 1,
                properties: {
                  leafy_greens: integerNonNegative,
                  microgreens: integerNonNegative,
                  herbs: integerNonNegative,
                  fruiting: integerNonNegative
                }
              },
              source: {
                type: 'string',
                enum: ['manual_override', 'derived']
              },
              notes: { type: 'string' }
            }
          },
          ...
          spatialContract: {
            type: 'object',
            description: 'Per-unit spatial + working-clearance contract consumed by the room layout solver.',
            required: ['unitFootprintM', 'workspaceClearanceM', 'orientation', 'plumbingSide'],
            additionalProperties: false,
            properties: {
              unitFootprintM: {
                type: 'object',
              ...
              workspaceClearanceM: {
                type: 'object',
                required: ['front', 'back', 'ends'],
                additionalProperties: false,
                properties: {
                  front: { type: 'number', minimum: 0 },
                  back: { type: 'number', minimum: 0 },
                  ends: { type: 'number', minimum: 0 }
                }
              },
              orientation: {
                type: 'string',
                enum: ['butt_end_to_end', 'double_sided', 'single_sided']
              },
              plumbingSide: {
                type: 'string',
                enum: ['wall', 'any']
