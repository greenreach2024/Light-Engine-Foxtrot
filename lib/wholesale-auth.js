...
export function verifyApiKey(farmId, apiKey) {
  const trustedCentralKey = (process.env.GREENREACH_API_KEY || '').trim();
  if (trustedCentralKey && apiKey === trustedCentralKey) {
    return true;
  }
  ...
  try {
    const keyBuf = Buffer.from(farmData.api_key, 'utf8');
