/**
 * Outdoor Conditions Validation Gateway
 * 
 * Validates outdoor weather data freshness and quality for ML operations.
 * Primary source: Weather API (Open-Meteo cached via fetchAndCacheWeather).
 * No physical outdoor sensor is required.
 * 
 * The validateOutdoorSensor() function accepts data from any source
 * (weather API, sensor, etc.) and validates range + freshness.
 * 
 * @module lib/outdoor-sensor-validator
 */

/**
 * Maximum age for outdoor weather data to be considered fresh (30 minutes)
 */
...
/**
 * Validate outdoor weather data freshness and quality
 * 
 * @param {Object} sensorData - Latest outdoor weather reading
 ...
 * @param {string} sensorData.zone - Weather data zone name
...
export function validateOutdoorSensor(sensorData) {
  ...
  if (!sensorData) {
    ...
    result.errors.push('No outdoor weather data available');
    return result;
  }
  ...
  if (!sensorData.timestamp) {
    result.isValid = false;
    result.isFresh = false;
    result.errors.push('Outdoor weather data missing timestamp');
    return result;
  }
  ...
  if (age > OUTDOOR_DATA_MAX_AGE_MS) {
    ...
    result.isFresh = false;
    result.errors.push(
      `Outdoor weather data is stale (${result.age_minutes} minutes old, max: 30 minutes)`
    );
  }
...
/**
 * Find outdoor weather reading from environmental data
 * 
 * @param {Array|Object} envData - Array of environmental readings or env object
 * @returns {Object|null} Latest outdoor weather reading or null
 */
...
/**
 * Gate ML operations based on outdoor weather data validity
 * 
 * @param {Object} validationResult - Result from validateOutdoorSensor()
 * @param {Object} [options] - Options
 * @param {boolean} [options.allowDegradedMode=true] - If true, allow ML to run in degraded mode without outdoor data
 * @returns {Object} Gate decision with message
 */
export function gateMLOperation(validationResult, options = {}) {
  const { allowDegradedMode = true } = options;
  ...
  if (!validationResult.isValid) {
    if (allowDegradedMode) {
      return {
        allowed: true,
        degraded: true,
        reason: 'outdoor_data_unavailable_degraded',
        message: `ML proceeding in degraded mode (indoor-only features): ${validationResult.errors.join('; ')}`,
  ...
  if (!validationResult.isFresh) {
    if (allowDegradedMode) {
      return {
        allowed: true,
        degraded: true,
        reason: 'outdoor_data_stale_degraded',
        message: `ML proceeding in degraded mode — outdoor data stale (${validationResult.age_minutes} min old)`,
    ...
    return {
      ...
      message: `Outdoor weather data is too old (${validationResult.age_minutes} minutes)`,
      age_minutes: validationResult.age_minutes
    };
  }
  ...
  return {
    ...
    message: 'Outdoor weather data is fresh and valid',
    age_minutes: validationResult.age_minutes
  };
}
