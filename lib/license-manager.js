/**
 * License Manager for Light Engine Farm Servers
 * 
 * Features:
...
export async function hasFeature(feature) {
  // Farm servers get all features (ml, analytics, automation, etc.)
  if (process.env.EDGE_MODE === 'true' || process.env.EDGE_MODE === true) {
    return true; // All features enabled on farm servers
  }
...
export async function getLicenseTier() {
  // Farm servers get 'edge' tier by default (includes ML/AI)
  if (process.env.EDGE_MODE === 'true' || process.env.EDGE_MODE === true) {
    console.log('[License] Farm server detected - using edge tier (includes ML/AI)');
    return 'edge';
  }
