 ...
 *     dehumidifier electrical (dehumLPerDay * 6.67 W/L/day peak/rated draw,
 *     benchmarked against commercial datasheets — see DEHUM_KW_PER_LPD below).
 *     This is peak/rated draw suitable for breaker sizing; monthly kWh requires
 *     a duty-cycle assumption and is left to the OPEX layer.
 */
...
// Latent heat of vaporization of water at ~20°C: 2454 kJ/kg. Converting to
// BTU: 2454 kJ/kg ÷ 1.05506 kJ/BTU = 2326 BTU/kg (rounded). Note: the VFC
// spec originally carried 1055 BTU/kg, which is actually the BTU-per-POUND
// value (1055 BTU/lb ≈ 2326 BTU/kg). This library uses the correct BTU/kg
// constant so `dailyWaterKg × constant` yields honest BTU/hr numbers; fixing
// the confusion roughly doubles cooling tonnage vs. the original spec's
// worked examples. See docs/features/VERTICAL_FARM_CALCULATOR_SPEC.md for
// the corrected table.
const LATENT_HEAT_BTU_PER_KG_WATER = 2326;
const HOURS_PER_DAY = 24;
const COOLING_KW_PER_TON = 1.2; // VFC: EER 10
// Dehumidifier peak electrical draw per L/day of capacity. Re-benchmarked
// 2026-04 against commercial datasheets (option C of the PR #15 dehum
// decision). The VFC spec originally said "300 L/day → ~400W" (1.33 W/L);
// this library previously interpreted that as "400W per 150 L/day" (2.67 W/L,
// yielding 800W for 300 L/day). Both understate real hardware by 2.5-5×.
//
//   Quest 155 Dual   73 L/day →   780 W  (10.7 W/L)
//   Quest 335       158 L/day →   960 W  ( 6.1 W/L)
//   Anden A95        44 L/day →   600 W  (13.6 W/L)
//   Anden A710V1    335 L/day → 1,175 W  ( 3.5 W/L)
//   Aprilaire E100   47 L/day →   720 W  (15.3 W/L)
//
// Mid-size commercial units cluster around 6-7 W/L/day; we use 6.67 W/L/day
// (2 kW for 300 L/day) as the rated/peak benchmark. This is what belongs in
// totalCircuitKW for breaker sizing. Actual monthly kWh requires a duty-cycle
// assumption (70% is a reasonable steady-state value in a transpiring room).
const DEHUM_KW_PER_LPD = 2 / 300; // 6.67 W/L/day peak; see comment above
...
export function computeLightingLoad(system) {
  ...
  if (typeof quantity !== 'number' || !Number.isFinite(quantity) || quantity <= 0) {
    throw new Error(
      `computeLightingLoad: quantity must be a positive finite number (got ${quantity === undefined ? 'undefined' : JSON.stringify(quantity)}) on template "${template?.id}"`
    );
  }
 ...
 *   latentBTUperHr  = dailyWaterKg × 2326 BTU/kg / 24h
 *   totalBTUperHr   = latentBTUperHr × (1 + sensibleHeatFactor)
 *   coolingTons     = totalBTUperHr / 12000
 *   dehumLPerDay    = dailyWaterKg  (1 kg water ≈ 1 L)
 *
 * Worked example (corrected): 10,000 plants × 30 g/day / 1000 = 300 kg/day;
 * 300 × 2326 / 24 = 29,075 latent BTU/hr; × 1.3 = 37,798 total; / 12000 =
 * 3.15 tons. The VFC spec's pre-fix number (1.4 tons) was wrong by a factor
 * of ~2.2 because it used 1055 BTU/lb as if it were BTU/kg.
 */
 ...
 *   dehumKW  = dehumLPerDay × 6.67 W/L/day      (peak/rated draw; see
 *                                                DEHUM_KW_PER_LPD comment for
 *                                                commercial datasheet refs)
 */
