...
export class HarvestPredictor {
  ...
  async predict(groupId, options = {}) {
    try {
      ...
      // Phase 4 #25: Confidence interval from historical variance
      const historicalVariances = this.getHistoricalVariances(crop);
      const predStdDev = historicalVariances.length > 1 ? this.calculateStdDev(historicalVariances) : 5;
      const earlyDate = new Date(seedDate);
      earlyDate.setDate(earlyDate.getDate() + Math.round(adjustedDays - 1.96 * predStdDev));
      const lateDate = new Date(seedDate);
      lateDate.setDate(lateDate.getDate() + Math.round(adjustedDays + 1.96 * predStdDev));
      ...
      return {
        ...
        confidenceInterval: {
          ...
          stdDevDays: Math.round(predStdDev * 100) / 100,
          level: 0.95,
          sampleSize: historicalVariances.length
        },
        factors,
        baseline: {
          days: baselineDays,
