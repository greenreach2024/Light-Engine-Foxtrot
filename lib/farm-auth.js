...
// JWT secret (must be explicit in production)
...
export function farmAuthMiddleware(req, res, next) {
  try {
    ...
    // 1. Try farm-sales portal JWT (type: farm_sales, snake_case claims)
    try {
      ...
      // Not a farm-sales token -- try farm admin auth
    }

    // 2. Check farm admin session tokens (edge/demo hex tokens)
    if (global.farmAdminSessions && global.farmAdminSessions.has(token)) {
      ...
      if (!session.expiresAt || new Date(session.expiresAt) > new Date()) {
        req.farm_id = session.farmId;
        req.user_id = session.email;
        req.user_role = session.role || 'admin';
        req.user_email = session.email;
        return next();
      }
    }

    // 3. Try farm admin JWT (camelCase claims, no type field)
    try {
      const decoded = jwt.verify(token, getJwtSecret());
      if (decoded.farmId || decoded.farm_id) {
        req.farm_id = decoded.farmId || decoded.farm_id;
        req.user_id = decoded.userId || decoded.user_id;
        req.user_role = decoded.role;
        req.user_email = decoded.email;
        return next();
      }
    } catch (_adminTokenErr) {
