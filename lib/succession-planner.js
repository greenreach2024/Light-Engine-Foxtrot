...
export class SuccessionPlanner {
  constructor(dataDir) {
    ...
    this.cropRegistryPath = path.join(dataDir, 'crop-registry.json');
    
    // Decision cache (5 min TTL)
    this.scheduleCache = new Map();
    this.cacheTTL = 300000; // 5 minutes
    this._cropRegistryCache = null;
  }

  /**
   * Load crop registry for CCA (Cut and Come Again) strategy lookup
   */
  loadCropRegistry() {
    if (this._cropRegistryCache) return this._cropRegistryCache;
    try {
      if (fs.existsSync(this.cropRegistryPath)) {
        const data = JSON.parse(fs.readFileSync(this.cropRegistryPath, 'utf-8'));
        this._cropRegistryCache = data.crops || {};
      } else {
        this._cropRegistryCache = {};
      }
    } catch (e) {
      console.warn('[SuccessionPlanner] Failed to load crop registry:', e.message);
      this._cropRegistryCache = {};
    }
    return this._cropRegistryCache;
  }

  /**
   * Get harvest strategy info for a crop from crop-registry.json
   * @returns {{ strategy: string, maxHarvests: number, regrowthDays: number, regrowthYieldFactor: number } | null}
   */
  getHarvestStrategy(cropName) {
    const registry = this.loadCropRegistry();
    const cropLower = cropName.toLowerCase();
    for (const [key, crop] of Object.entries(registry)) {
      if (crop.growth?.harvestStrategy === 'cut_and_come_again') {
        if (key.toLowerCase() === cropLower || cropLower.includes(key.toLowerCase()) || key.toLowerCase().includes(cropLower)) {
          return {
            ...
            maxHarvests: crop.growth.maxHarvests || 4,
            regrowthDays: crop.growth.regrowthDays || 14,
            regrowthYieldFactor: crop.growth.regrowthYieldFactor || 0.85
          };
        }
      }
    }
  ...
  async generateSchedule(params) {
    ...
    try {
      ...
      const ccaStrategy = this.getHarvestStrategy(params.crop);
      
      // For CCA crops, each planting produces multiple harvests over an extended window.
      // Effective production span = growthDays + (maxHarvests - 1) × regrowthDays
      // This means fewer succession plantings are needed.
      let effectiveSuccessionGap = successionGap;
      if (ccaStrategy) {
        const productionSpan = growthDays + (ccaStrategy.maxHarvests - 1) * ccaStrategy.regrowthDays;
        // Space succession plantings by the full production span to avoid over-planting
        effectiveSuccessionGap = Math.max(successionGap, productionSpan);
        console.log(`[SuccessionPlanner] CCA crop "${params.crop}": ${ccaStrategy.maxHarvests} harvests, ` +
          `regrowth ${ccaStrategy.regrowthDays}d, production span ${productionSpan}d, ` +
          `effective gap ${effectiveSuccessionGap}d`);
      }
      ...
      for (let week = 0; week < weeks; week++) {
        ...
        harvestDate.setDate(harvestDate.getDate() + (week * effectiveSuccessionGap));
        ...
        schedule.push({
          ...
          successionGap: effectiveSuccessionGap,
          ...
          // CCA fields (null for single-harvest crops)
          harvestStrategy: ccaStrategy ? ccaStrategy.strategy : 'single_harvest',
          ccaInfo: ccaStrategy ? {
            maxHarvests: ccaStrategy.maxHarvests,
            regrowthDays: ccaStrategy.regrowthDays,
            regrowthYieldFactor: ccaStrategy.regrowthYieldFactor,
            harvestDates: Array.from({ length: ccaStrategy.maxHarvests }, (_, i) => {
              const d = new Date(harvestDate);
              d.setDate(d.getDate() + i * ccaStrategy.regrowthDays);
              return d.toISOString().split('T')[0];
            }),
            totalYieldMultiplier: Array.from({ length: ccaStrategy.maxHarvests }, (_, i) =>
              Math.pow(ccaStrategy.regrowthYieldFactor, i)
            ).reduce((a, b) => a + b, 0)
          } : null
        });
      ...
      const result = {
        ...
        harvestStrategy: ccaStrategy ? {
          strategy: 'cut_and_come_again',
          maxHarvests: ccaStrategy.maxHarvests,
          regrowthDays: ccaStrategy.regrowthDays,
          regrowthYieldFactor: ccaStrategy.regrowthYieldFactor,
          effectiveSuccessionGap: effectiveSuccessionGap,
          note: `CCA crop: ${ccaStrategy.maxHarvests} harvests per planting, ` +
                `${ccaStrategy.regrowthDays}d between cuts. Succession gap widened ` +
                `from ${successionGap}d to ${effectiveSuccessionGap}d.`
        } : { strategy: 'single_harvest' },
        summary: {
          totalWeeks: weeks,
