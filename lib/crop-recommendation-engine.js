...
/**
 * Calculate demand velocity score (placeholder - will integrate with sales data in Phase 2)
 * @param {string} cropId
 * @returns {number} Score 0-100
 */
function scoreDemandVelocity(cropId) {
  // Phase 1: Simple heuristic based on common crops
  const popularCrops = [
...
async function generateCropRecommendations(params) {
  ...
  for (const cropId of availableCrops) {
    ...
    const demandScore = scoreDemandVelocity(cropId);
