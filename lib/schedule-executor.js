...
export class ScheduleExecutor {
  constructor(options = {}) {
    ...
    this.minPhotoperiodHours = Number.isFinite(options.minPhotoperiodHours)
      ? options.minPhotoperiodHours
      : 12;
    this.maxTargetPPFD = Number.isFinite(options.maxTargetPPFD)
      ? options.maxTargetPPFD
      : 2000;
  ...
  async tick() {
    ...
    try {
      ...
      // Load data (including rooms for room-level schedule inheritance)
      const [groups, plans, schedules, rooms] = await Promise.all([
        this.loadGroups(),
        this.loadPlans(),
        this.loadSchedules(),
        this.loadRooms()
      ...
      console.log(`[ScheduleExecutor] Loaded ${groups.length} groups, ${plans.length} plans, ${schedules.length} schedules, ${rooms.length} rooms`);
      
      // Process each group
      const results = [];
      for (const group of groups) {
        try {
          const result = await this.processGroup(group, plans, schedules, now, rooms);
   ...
   * @param {Object} group - Group record
   * @param {Array} plans - Available plans/recipes
   * @param {Array} schedules - Available schedules
   * @param {Date} now - Current time
   * @param {Array} rooms - Room records (for schedule inheritance)
   */
  async processGroup(group, plans, schedules, now, rooms = []) {
    console.log(`[ScheduleExecutor] Processing group ${group.id || group.name}, plan=${group.plan}, schedule=${group.schedule}, status=${group.status}`);
    
    // Skip non-deployed groups (draft groups should not receive schedule commands)
    if (group.status && group.status !== 'deployed') {
      console.log(`[ScheduleExecutor] Skipping group ${group.id} - status is '${group.status}' (not deployed)`);
      return null;
    }
    
    // A plan (recipe) is required — without it we don't know what spectrum/PPFD to apply
    if (!group.plan) {
      console.log(`[ScheduleExecutor] Skipping group ${group.id} - missing plan (recipe not yet selected)`);
      return null;
    }
    
    // Schedule resolution: group-level schedule takes priority, then inherit from room
    let effectiveScheduleId = group.schedule;
    if (!effectiveScheduleId) {
      // Inherit schedule from the group's room
      const roomName = group.room || '';
      const room = rooms.find(r =>
        r.name === roomName || r.id === roomName ||
        (r.name && r.name.toLowerCase() === roomName.toLowerCase())
      );
      if (room?.grouping?.scheduleId) {
        effectiveScheduleId = room.grouping.scheduleId;
        console.log(`[ScheduleExecutor] Group ${group.id}: inheriting schedule '${effectiveScheduleId}' from room '${room.name}'`);
      }
    }
    
    if (!effectiveScheduleId) {
      console.log(`[ScheduleExecutor] Skipping group ${group.id} - no schedule (group or room)`);
      return null;
    }
    ...
    console.log(`[ScheduleExecutor] Group ${group.id}: hasLights=${hasLights}, hasController=${!!hasController}`);
    
    if (!hasLights && !hasController) {
      console.log(`[ScheduleExecutor] Skipping group ${group.id} - no lights or controller`);
      return null;
    }
  ...
  const plan = await this.resolvePlan(plans, group);
    console.log(`[ScheduleExecutor] Loaded ${schedules.length} schedules:`, schedules.map(s => s.id));
    let schedule = schedules.find(s => s.id === effectiveScheduleId || s.groupId === group.id);
    if (!schedule) {
      schedule = this.buildScheduleFromPlanConfig(group);
      if (schedule) {
        console.log(`[ScheduleExecutor] Group ${group.id}: using embedded Group V2 schedule`);
      }
    }
    console.log(`[ScheduleExecutor] Group ${group.id}: found schedule=${!!schedule}, looking for schedule=${effectiveScheduleId}`);
    
    if (!schedule) {
      console.warn(`[ScheduleExecutor] Schedule ${effectiveScheduleId} not found for group ${group.id}`);
      return null;
    }
    ...
    if (active) {
      ...
      if (!recipe) {
        ...
        const photoperiodHours = this.resolvePhotoperiodHours(schedule, plan);
        const safePhotoperiod = Math.max(this.minPhotoperiodHours, photoperiodHours || this.minPhotoperiodHours);

        const dliTarget = Number(recipe.dli ?? recipe.dli_target ?? 0);
        const ppfdFromRecipe = Number(recipe.ppfd ?? recipe.ppfd_target ?? 0);

        let targetPPFD = ppfdFromRecipe > 0 ? ppfdFromRecipe : 100;
        if (Number.isFinite(dliTarget) && dliTarget > 0) {
          targetPPFD = (dliTarget * 1_000_000) / (safePhotoperiod * 3600);
        }

        if (!Number.isFinite(targetPPFD) || targetPPFD <= 0) {
          targetPPFD = 100;
        }

        if (targetPPFD > this.maxTargetPPFD) {
          console.warn(`[ScheduleExecutor] Target PPFD ${targetPPFD.toFixed(1)} exceeds max ${this.maxTargetPPFD}, clamping`);
          targetPPFD = this.maxTargetPPFD;
        }

        // Convert recipe (blue, green, red percentages) to hardware channels (bl, rd, ww, cw)
        console.log(`[ScheduleExecutor] Group ${group.id}: photoperiod=${safePhotoperiod}h, dli=${dliTarget || 'n/a'}, targetPPFD=${targetPPFD.toFixed(1)}`);
  ...
  buildScheduleFromPlanConfig(group) {
    const cfg = group?.planConfig?.schedule;
    if (!cfg || typeof cfg !== 'object') return null;

    const cycles = Array.isArray(cfg.cycles) && cfg.cycles.length > 0
      ? cfg.cycles
      : cfg.cycleA
        ? [{
            start: cfg.startTime || cfg.cycleA?.start,
            off: cfg.offTime || cfg.cycleA?.off,
            photo: cfg.photoperiodHours || cfg.durationHours || cfg.cycleA?.photo,
            ramp: {
              up: cfg.rampUpMin ?? cfg.cycleA?.ramp?.up,
              down: cfg.rampDownMin ?? cfg.cycleA?.ramp?.down
            },
            spectrum: null
          }]
        : [];
    ...
    return {
      id: `group:${group.id}`,
      groupId: group.id,
      name: `${group.name || group.id} Schedule`,
      mode: cfg.mode || 'one',
      timezone: cfg.timezone,
      photoperiodHours: cfg.photoperiodHours || cfg.durationHours || cfg.totalOnHours,
      ramp: {
        up: cfg.rampUpMin,
        down: cfg.rampDownMin
      },
      cycles,
  ...
  resolvePhotoperiodHours(schedule, plan) {
    if (!schedule) return plan?.defaults?.photoperiod || 12;

    if (Number.isFinite(schedule.photoperiodHours)) {
      return schedule.photoperiodHours;
    }

    if (Array.isArray(schedule.cycles) && schedule.cycles.length > 0) {
      const cycle = schedule.cycles[0];
      if (Number.isFinite(cycle.photo)) return cycle.photo;
      if (cycle.start && cycle.off) {
        const [sh, sm] = cycle.start.split(':').map(Number);
        const [oh, om] = cycle.off.split(':').map(Number);
        if ([sh, sm, oh, om].every(n => Number.isFinite(n))) {
          const startMinutes = sh * 60 + sm;
          const offMinutes = oh * 60 + om;
          let delta = offMinutes - startMinutes;
    ...
    return plan?.defaults?.photoperiod || 12;
  }
  ...
  async controlLight(light, status, hexPayload) {
    console.log(`[ScheduleExecutor] controlLight called with light=`, JSON.stringify(light));
  ...
  async controlGrow3Light(light, status, hexPayload) {
    ...
    console.log(`[ScheduleExecutor] Looking up ${lightId} in deviceRegistry:`, JSON.stringify(this.deviceRegistry));
  ...
  async controlKasaPlug(light, status) {
    ...
    const payload = { state };
    payload.source = 'schedule-executor';
    payload.sessionId = light.groupId || light.id || host;
  ...
  async controlSwitchBotPlug(light, status) {
    ...
    const payload = {
      ...
      sessionId: light.groupId || light.id || deviceId,
      confirmed: true
    };
  ...
  /**
   * Load rooms from JSON file (for room-level schedule inheritance)
   */
  async loadRooms() {
    try {
      const filePath = path.join(this.dataDir, 'rooms.json');
      const fileContent = await fs.readFile(filePath, 'utf8');
      const data = JSON.parse(fileContent);
      return data.rooms || [];
    } catch (error) {
      if (error.code === 'ENOENT') {
        console.warn('[ScheduleExecutor] rooms.json not found, returning empty array');
        return [];
      }
      console.error('[ScheduleExecutor] Failed to load rooms:', error);
      return [];
    }
  }
