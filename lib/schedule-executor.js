...
export class ScheduleExecutor {
  ...
  async runMLAnomalyDetection() {
    ...
    try {
      ...
      if (result.success && result.anomalies) {
        ...
        console.log(`[ML] [OK] Detected ${result.count} anomalies in ${duration}ms (${critical_count} critical, ${warning_count} warning, ${info_count} info)`);
        ...
        important.forEach(anomaly => {
          ...
          console.warn(`[ML] [WARNING]  ${anomaly.severity.toUpperCase()}: ${anomaly.zone} - ${anomaly.reason} (${outdoorContext})`);
        });
      ...
      console.error('[ML] [ERROR] Anomaly detection failed:', error.message);
      throw error;
    }
  }
  ...
  async resolvePlan(plans, group) {
    ...
    if (plan) {
      console.log(`[ScheduleExecutor] [OK] Found plan "${plan.name}" (id: ${plan.id}) for group "${group?.name}"`);
      return plan;
    }
    ...
    try {
      ...
      if (recipePlan) {
        console.log(`[ScheduleExecutor] [OK] Found plan via lighting-recipes fallback for group "${group?.name}"`);
        return recipePlan;
      }
    } catch (err) {
    ...
    console.error(`[ScheduleExecutor] [FAIL] No plan found for group "${group?.name}" with tokens:`, tokens);
