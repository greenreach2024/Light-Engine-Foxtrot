...
export class ScheduleExecutor {
  ...
  async processGroup(group, plans, schedules, now) {
    console.log(`[ScheduleExecutor] Processing group ${group.id || group.name}, plan=${group.plan}, schedule=${group.schedule}`);
    
    // Skip groups without plan or schedule
    if (!group.plan || !group.schedule) {
      console.log(`[ScheduleExecutor] Skipping group ${group.id} - missing plan or schedule`);
      return null;
    }
  ...
  const plan = await this.resolvePlan(plans, group);
    ...
    let schedule = schedules.find(s => s.id === group.schedule || s.groupId === group.id);
    if (!schedule) {
    ...
    console.log(`[ScheduleExecutor] Group ${group.id}: found schedule=${!!schedule}, looking for schedule=${group.schedule}`);
