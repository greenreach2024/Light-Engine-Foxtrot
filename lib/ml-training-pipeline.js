...
function trainCropModel(records) {
  const featureNames = ['ppfd', 'blue_pct', 'red_pct', 'temp_c', 'humidity_pct', 'grow_days'];
  
  // Build feature matrix
  const rows = [];
  for (const rec of records) {
    const rp = rec.recipe_params_avg || {};
    const weight = rec.outcomes?.weight_per_plant_oz;
    if (weight == null || weight <= 0) continue;
    
    const features = [
      parseFloat(rp.ppfd) || 0,
      parseFloat(rp.blue_pct) || 0,
      parseFloat(rp.red_pct) || 0,
      parseFloat(rp.temp_c) || 0,
      parseFloat(rp.humidity_pct) || 0,
      parseInt(rec.grow_days) || 0
    ];
    
    // Skip if all features are zero
    if (features.every(f => f === 0)) continue;
    rows.push({ features, target: weight });
  }
