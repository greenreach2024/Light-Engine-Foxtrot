...
import { ensureFarmConfigFile } from './farm-config-bootstrap.js';
    ...
    try {
      if (fs.existsSync(this.persistPath)) {
        ...
        for (const item of items) {
          if (item.farm_id && item._entity_id) {
            const key = this._key(item.farm_id, item._entity_id);
            this.store.set(key, item);
          }
        }
        console.log(`[farm-store] Loaded ${items.length} ${this.entityName} from ${this.persistPath}`);
      }
    } catch (err) {
      console.error(`[farm-store] Failed to load ${this.entityName} from file:`, err.message);
    }
  }

  /**
   * Persist data to JSON file (debounced — writes at most once per second)
   */
  _persistToFile() {
    ...
    this._persistTimer = setTimeout(() => {
      this._persistTimer = null;
      try {
        const dirPath = path.dirname(this.persistPath);
        if (!fs.existsSync(dirPath)) fs.mkdirSync(dirPath, { recursive: true });
        const items = Array.from(this.store.values());
        fs.writeFileSync(this.persistPath, JSON.stringify({ entity: this.entityName, updated_at: new Date().toISOString(), items }, null, 2), 'utf8');
      } catch (err) {
        console.error(`[farm-store] Failed to persist ${this.entityName}:`, err.message);
      }
    }, 1000);
  }
 ...
 */
const POS_ORDERS_PATH = path.resolve(__dirname, '../data/pos-orders.json');

export const farmStores = {
  inventory: new FarmScopedStore('inventory', { persistPath: path.resolve(__dirname, '../data/pos-inventory.json') }),
  orders: new FarmScopedStore('orders', { persistPath: POS_ORDERS_PATH }),
  payments: new FarmScopedStore('payments', { persistPath: path.resolve(__dirname, '../data/pos-payments.json') }),
...
/**
 * Lazy initialization - call when farm is first accessed
 * Idempotent - safe to call multiple times for same farm
 */
export function ensureFarmInitialized(farmId) {
  // Check if farm already has data
  if (farmStores.inventory.countForFarm(farmId) > 0) {
    return; // Already initialized
  }
  
  console.log(`[farm-store] Lazy-initializing farm: ${farmId}`);
  
  // Initialize only this farm's data
  loadRealCropInventory(farmId);
  initializeFarmDemoData(farmId);
}

/**
 * Initialize current farm from farm.json on server startup
 * Only initializes ONE farm - the actual production farm
 */
function initializeCurrentFarm() {
  try {
    ensureFarmConfigFile({ filePath: FARM_FILE });
    const farmData = JSON.parse(fs.readFileSync(FARM_FILE, 'utf8'));
    const currentFarmId = farmData.farmId || 'light-engine-demo';
    console.log(`[farm-store] Initializing current farm only: ${currentFarmId}`);
    
    // Load real crop inventory for current farm
    loadRealCropInventory(currentFarmId);
    
    // Initialize programs for current farm
    initializeFarmDemoData(currentFarmId);
    
  } catch (error) {
    console.error('[farm-store] Failed to load farm.json, no auto-initialization');
  }
}

// Initialize ONLY the current farm at module load
initializeCurrentFarm();
