// Global Demo Mode Fetch Interceptor for AWS S3 Static Hosting
(function() {
  if (window.DEMO_MODE || localStorage.getItem('demoMode') === 'true') {
    if (window._demoFetchInterceptorInstalled) return; // Already installed
    
    const originalFetch = window.fetch;
    window.fetch = function(...args) {
      const url = String(args[0] || '');
      
      // Stub responses for common API endpoints
      const stubMap = {
        '/env': { rooms: {}, targets: {}, control: {}, history: [] },
        '/plans.json': [],
        '/plans': [],
        '/sched': [],
        '/energy-forecast': { 
          ok: true,
          forecast: [
            { hour: '00:00', kwh: 12.3, cost: 1.85 },
            { hour: '06:00', kwh: 14.5, cost: 2.18 },
            { hour: '12:00', kwh: 20.5, cost: 3.08 },
            { hour: '18:00', kwh: 16.2, cost: 2.43 }
          ]
        },
        '/rules': [],
        '/statistics': { 
          ok: true,
          alerts: { critical: 0, warnings: 0, info: 0 },
          farm: { total_devices: 0, total_rooms: 2, total_zones: 8 }
        },
        '/status': { ok: true, status: 'demo', systemStatus: 'online' },
        '/ml-anomalies': { ok: true, anomalies: [], history: [] },
        '/weather': { 
          ok: true,
          current: {
            temperature_f: 72,
        ...
        '/history': [],
        '/healthz': { status: 'ok', mode: 'demo' },
        '/devicedatas': [],
        '/catalog': []
      };
      
      for (const [endpoint, stubData] of Object.entries(stubMap)) {
        if (url.includes(endpoint)) {
          return Promise.resolve(new Response(JSON.stringify(stubData), {
            status: 200,
    ...
    window._demoFetchInterceptorInstalled = true;
    console.log('[Global Demo Mode] Fetch interceptor installed');
  }
})();
...
async function fetchSchedulesDocument() {
  // Demo mode: return empty schedules
  if (window.DEMO_MODE || localStorage.getItem('demoMode') === 'true') {
    return [];
  }
  try {
...
async function api(url, opts = {}) {
  // Demo mode: For AWS S3 static hosting, gracefully handle missing API endpoints
  if (window.DEMO_MODE || localStorage.getItem('demoMode') === 'true') {
    // Define stub responses for API endpoints that don't have static files
    const stubResponses = {
      '/intro-cards': {},
      '/weather': { status: 'demo', current: {}, forecast: [] },
      '/plans': [],
      '/sched': [],
      '/healthz': { status: 'ok', mode: 'demo' },
      '/devicedatas': [],
    ...
    // Check for stub response
    for (const [endpoint, stubData] of Object.entries(stubResponses)) {
      if (url.includes(endpoint)) {
        return stubData;
      }
    }
    
    // Map API endpoints to existing demo data files
    const demoDataMap = {
      '/env': '/data/env.json'
    };
    
    // Check if this is a known demo endpoint with static data
    for (const [apiPath, dataPath] of Object.entries(demoDataMap)) {
      if (url.includes(apiPath)) {
        try {
          const resp = await fetch(dataPath);
          if (resp.ok) return await resp.json();
        } catch (error) {
          console.warn(`[Demo Mode] Could not load ${dataPath}, using empty data`);
        }
      }
    }
  }
...
class FarmWizard {
  ...
  async loadWeather(lat, lng) {
    ...
    // Demo mode: skip weather loading
    if (window.DEMO_MODE || localStorage.getItem('demoMode') === 'true') {
      weatherContent.innerHTML = '<div style="text-align: center; color: #666;">Weather data unavailable in demo mode</div>';
...
async function pollHealthz() {
  // Demo mode: skip health check
  if (window.DEMO_MODE || localStorage.getItem('demoMode') === 'true') {
    const statusEl = document.getElementById('communicationStatus');
    if (statusEl) {
      statusEl.textContent = 'Demo Mode';
      statusEl.className = 'communication-status demo';
      statusEl.title = 'Running in demo mode';
    }
    return;
  }
  try {
