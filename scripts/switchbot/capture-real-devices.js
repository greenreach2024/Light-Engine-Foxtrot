...
async function waitForApiAccess() {
  console.log('[TIME] Waiting for SwitchBot API rate limit to clear...');
  ...
  while (true) {
    ...
    try {
      ...
      if (response.status === 200 && response.body.statusCode === 100) {
        console.log('[OK] API access restored!');
        return response.body;
      }
      
      console.log(`[WARNING]  Unexpected response: ${response.status}`, response.body);
...
async function main() {
  console.log('[START] SwitchBot Real Device Capture');
  console.log('================================');
  ...
  try {
    ...
    if (deviceList.length === 0) {
      console.log('[WARNING]  No devices found in your SwitchBot account');
      return;
    }
    
    console.log(`[SUCCESS] SUCCESS! Found ${deviceList.length} real SwitchBot devices:`);
    console.log('================================================');
    ...
    console.log('\\n[REFRESH] Updating server cache with real devices...');
    try {
      const http = await import('http');
      const req = http.request('http://localhost:8091/api/switchbot/devices?refresh=1', (res) => {
        console.log(`[OK] Server cache updated (HTTP ${res.statusCode})`);
      });
      req.on('error', (e) => {
        console.log(`[WARNING]  Could not update server cache: ${e.message}`);
      });
      req.end();
    } catch (e) {
      console.log('[WARNING]  Server may not be running - cache update skipped');
    }
