...
(async () => {
  try {
    ...
    console.log(`   [OK] Persisted ${persistResult.added} anomalies (total: ${persistResult.total})`);
    ...
    for (const anomaly of simulatedAnomalies) {
      ...
      if (response && response.action !== 'none') {
        ...
        console.log(`   [OK] ${response.action} for ${anomaly.zone}`);
    ...
    console.log(`   [OK] Found ${recentActions.length} actionable anomalies`);
    ...
    console.log(`   [OK] Cooldown updated to ${updatedConfig.action_cooldown_minutes} minutes`);
    ...
    if (forecastResponse) {
      console.log(`   [OK] Proactive action: ${forecastResponse.action}`);
...
[OK] All Integration Tests Passed:
  ...
  [OK] Anomaly persistence working
  [OK] Automation response evaluation working
  [OK] Configuration management working
  [OK] Cooldown mechanism working
  [OK] Manual evaluation (API simulation) working
  [OK] Forecast-based proactive automation working
