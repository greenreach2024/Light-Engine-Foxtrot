...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!isAuthenticated()) {
        console.warn('[WARN]️ Not authenticated, redirecting to login...');
...
async function autoAuthenticateLocal() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            console.warn('[WARN]️ Auto-authentication failed');
            // Redirect to login if auto-auth fails
            window.location.href = '/login.html';
        }
    } catch (error) {
        console.warn('[WARN]️ Auto-authentication error:', error.message);
        window.location.href = '/login.html';
    }
}
