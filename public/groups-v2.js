  ...
  if (applyPlanBtn) {
    applyPlanBtn.addEventListener('click', () => {
      ...
      if (typeof showToast === 'function') {
        ...
        showToast({ title: 'Plan Applied', msg: summary, kind: 'success', icon: '[OK]' });
      }
    });
  }
        ...
        if (typeof showToast === 'function') {
          if (errorCount === 0) {
            showToast({ 
              ...
              msg: `[OK] All ${successCount} light(s) in ${groupLabel} blinked successfully!`, 
              kind: 'success',
              icon: '🔦'
            }, 3000);
            ...
            showToast({ 
              ...
              msg: `[ERROR] None of the ${errorCount} light(s) in ${groupLabel} are responding. Check connections and power.`, 
              kind: 'error',
              icon: '[WARNING]'
            }, 4000);
          } else {
...
function renderLightInfoCard(light) {
  ...
  if (isDynamic) {
    ...
    html += '<div style="font-weight: 700; color: #1e40af; margin-bottom: 4px;">[OK] TUNABLE SPECTRUM</div>';
    ...
    html += '<div style="font-weight: 700; color: #92400e; margin-bottom: 4px;">[WARNING] STATIC SPECTRUM</div>';
...
function updateGroupsV2StatusBadge(group) {
  ...
  if (status === 'deployed') {
    badge.textContent = '[OK] DEPLOYED';
    badge.style.background = 'linear-gradient(135deg,#10b981 0%,#059669 100%)';
    badge.style.color = 'white';
  } else {
    badge.textContent = '[NOTE] DRAFT';
...
async function saveGroupsV2Group(status = 'draft') {
  ...
  try {
    ...
    if (typeof showToast === 'function') {
      showToast({ title: 'Save Failed', msg: `Could not save to server: ${error.message}`, kind: 'error', icon: '[ERROR]' });
    } else {
  ...
  if (typeof showToast === 'function') {
    ...
    const statusIcon = status === 'deployed' ? '[START]' : '💾';
    ...
    const icon = scheduleMessage.includes('failed') ? '[WARNING]' : statusIcon;
  ...
  if (saveBtn) {
    saveBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        console.warn('[groups-v2] Failed to upsert schedule', error);
        if (typeof showToast === 'function') {
          showToast({ title: 'Schedule not saved', msg: error?.message || 'Failed to sync schedule.', kind: 'warn', icon: '[WARNING]' });
        }
        scheduleMessage = ' • Schedule sync failed';
      }
      ...
      if (typeof showToast === 'function') {
        ...
        if (scheduleMessage.includes('failed')) {
          showToast({ title: 'Group Saved', msg: `${details.join(' • ')}${scheduleMessage}`, kind: 'warn', icon: '[WARNING]' });
        } else {
          showToast({ title: 'Group Saved', msg: `${details.join(' • ')}${scheduleMessage}`, kind: 'success', icon: '[OK]' });
        }
      }
      if (statusEl) {
...
function populateGroupsV2UnassignedLightsDropdown() {
  ...
  unassigned.forEach((light) => {
    ...
    const typeIcon = light.isPlug ? '🔌 ' : light.fromIoT ? '[IDEA] ' : '';
...
function applyPlanSelectionToCurrentGroup(planOverride) {
  ...
  if (typeof window.showToast === 'function') {
    ...
    window.showToast({ title: 'Plan Saved', msg: `Applied ${label} to group ${group.name || group.id}`, kind: 'success', icon: '[OK]' }, 1500);
  }
  renderGroupsV2LightCard(plan);
}
    ...
    try {
      ...
      card.innerHTML = `
        <div class="groups-v2-lightcard-fallback">
          <div class="text-warning" style="font-size:1rem;margin-bottom:0.5em;">[WARNING] Spectrum/factory data missing for one or more assigned lights.</div>
...
function populateGroupsV2LoadGroupDropdown() {
  ...
  sortedGroups.forEach(group => {
    ...
    const status = group.status || 'draft';
    const statusIcon = status === 'deployed' ? '[OK]' : '[NOTE]';
